...
A key idea behind this package is the idea of making it easier to access, *know* what data is available and make it easier to analyse.
...
# Religious association in New Zealand
...
By cleaned, the census columns are split up into year, topic and variable columns. 
...
The above workflow essentially encapsulates the workflow brought to the user by the `nzcenrs` package which makes it easy to access the data, select the topics wanted, filter by regions and transform in a 'tidy' table.
...
For comparison sake, let us create a percentage of people as opposed to absolute numbers.
...
```{r region_line_plot, fig.width=7, fig.height=7}
...
Interesting, in these plots you can clearly see the 'decline' of Christianity occuring the increasing 'lack of religion'. 
...
```{r minor_religions, fig.width=6, fig.height=6}
